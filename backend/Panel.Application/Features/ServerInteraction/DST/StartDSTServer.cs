...
namespace Panel.Application.Features.ServerInteraction.DST
{
	...
	public class StartDSTServerHandler : IRequestHandler<StartDSTServer, IActionResult>
	{
		...
		public async Task<IActionResult> Handle(StartDSTServer request, CancellationToken cancellationToken)
		{
			...
			await _processManager.ExecuteCommandAsync($"docker run -v {serverDirectory}:/data --name {username}DST -i " +
			...
			_processManager.ExecuteCommand($"docker start {username}DST");

			var serverProcess = _processManager.CreateCmdProcess($"docker logs -f {username}DST");
			...
			_ = Task.Run(async () =>
			{
				...
				var processManager = scope.ServiceProvider.GetRequiredService<IOsInteraction>();
				processManager.ExecuteCommand($"docker rm {username}DST");
